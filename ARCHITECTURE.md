...
- Containerize with Docker, add CI for linting/tests, and build a simple helm chart for k8s deployment.
...
Client (browser) --> FastAPI (main.py)
    ...
    - Calls acaps_data.py (external API + in-memory cache)
    - Calls context_ai.py -> anthropic (external) or world_risk.py (static)
