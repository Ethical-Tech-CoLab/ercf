...
// computed/business value, only display labels for entities the
// API already returned (same convention as ERCF's app.js RISK_DEF
// label dictionaries).
// ═══════════════════════════════════════════════════════════
...
const state = {
  ...
  pathways: {},
  gatekeeperPathwayId: null,
  confMult: 0.70, // 1 - uncertaintyLevel; shared across cost and readiness layers (DESIGN.md §7)
  charts: {},
};

const PATHWAY_CATEGORY_LABELS = {
  local_integration: 'Local Integration (Malawi)',
  voluntary_return: 'Voluntary Return',
  third_country_resettlement: 'Third-Country Resettlement',
};

const PATHWAY_NAMES = {
  malawi_local_integration: 'Malawi — Local Integration (Work-Rights Pathway)',
  voluntary_return_drc: 'Voluntary Return — DRC',
  voluntary_return_burundi: 'Voluntary Return — Burundi',
  voluntary_return_rwanda: 'Voluntary Return — Rwanda',
  resettlement_usa: 'Third-Country Resettlement — USA',
  resettlement_canada: 'Third-Country Resettlement — Canada',
  resettlement_eu: 'Third-Country Resettlement — EU',
};

const GATEKEEPER_ORDER = ['legal_consent', 'host_willingness', 'security'];
...
function gatekeeperBadge(status, apiLabel) {
  // Deliberate deviation from the 3-way scheme (verde/amber/vermelho) requested:
  // 'partial' gets its own amber label rather than being folded into 'Unknown',
  // preserving the report's own Unknown-vs-(Un)willing precision principle
  // (§3.5/§5) — merging two distinct epistemic states would be exactly the
  // mistake that principle exists to prevent.
  if (status === 'operational') return { text: 'Confirmed', cls: 'gk-green' };
  if (status === 'partial')     return { text: 'Partially confirmed', cls: 'gk-amber' };
  if (status === 'unknown')     return { text: 'Unknown', cls: 'gk-amber' };
  if (status === 'blocked')     return { text: apiLabel, cls: 'gk-red' }; // apiLabel is "Blocked" or "Unwilling", from status_label() server-side
  return { text: apiLabel || status, cls: 'gk-amber' };
}

function sourcingBadge(tag) {
  if (tag === 'validated')   return { text: 'Validated', cls: 'src-validated' };
  if (tag === 'estimated')   return { text: 'Estimated', cls: 'src-estimated' };
  return { text: 'Pending sourcing', cls: 'src-unvalidated' };
}
...
async function init() {
  const [vocab, cohorts, pathways] = await Promise.all([
  ...
  state.gatekeeperPathwayId = Object.keys(pathways)[0] || null;

  populateCohortSelect();
  populateGatekeeperPathwaySelect();
  ...
  document.getElementById('gatekeeperPathwaySelect').addEventListener('change', e => {
    state.gatekeeperPathwayId = e.target.value;
    updateGatekeeperCards();
  });
...
function populateGatekeeperPathwaySelect() {
  const sel = document.getElementById('gatekeeperPathwaySelect');
  sel.innerHTML = '';
  Object.keys(state.pathways).forEach(pid => {
    const opt = document.createElement('option');
    opt.value = pid;
    opt.textContent = PATHWAY_NAMES[pid] || pid;
    if (pid === state.gatekeeperPathwayId) opt.selected = true;
    sel.appendChild(opt);
  });
}
...
// ═══════════════════════════════════════════════════════════
// PATHWAY SOURCING CARDS (item 5)
// ═══════════════════════════════════════════════════════════

function renderPathwayCards() {
  const container = document.getElementById('pathwayCategoryCards');
  container.innerHTML = '';

  Object.entries(PATHWAY_CATEGORY_LABELS).forEach(([category, label]) => {
    const members = Object.entries(state.pathways).filter(([, p]) => p.category === category);

    const col = document.createElement('div');
    col.className = 'col-md-4';

    const rows = members.map(([pid, p]) => {
      const badge = sourcingBadge(p.overall_tag);
      const capNote = p.capacity == null
        ? `<div class="pathway-pending">Capacity: pending sourcing</div>`
        : `<div class="pathway-pending">Capacity: ${p.capacity.toLocaleString()}</div>`;
      return `
        <div class="pathway-row" style="display:block">
          <div class="d-flex justify-content-between align-items-center">
            <span>${PATHWAY_NAMES[pid] || pid}</span>
            <span class="src-badge ${badge.cls}">${badge.text}</span>
          </div>
          ${capNote}
        </div>`;
    }).join('');
    ...
    col.innerHTML = `
      <div class="card h-100">
        ...
        <div class="card-body">
          ${rows || '<div class="empty-state">No pathways defined in this category.</div>'}
        </div>
      </div>`;
...
// ═══════════════════════════════════════════════════════════
// GATEKEEPER CARDS (item 4)
// ═══════════════════════════════════════════════════════════

async function updateGatekeeperCards() {
  if (!state.gatekeeperPathwayId) return;

  const res = await fetch('/api/readiness/gatekeepers', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ pathway_id: state.gatekeeperPathwayId, conf_mult: state.confMult }),
  });
  if (!res.ok) return;
  const data = await res.json();

  const container = document.getElementById('gatekeeperCards');
  container.innerHTML = '';

  GATEKEEPER_ORDER.forEach(fid => {
    const f = data.factors[fid];
    if (!f) return;
    const badge = gatekeeperBadge(f.status, f.label);
    const sourcing = sourcingBadge(f.tag);
    const borderCls = badge.cls === 'gk-green' ? 'gk-border-green' : badge.cls === 'gk-red' ? 'gk-border-red' : 'gk-border-amber';

    const col = document.createElement('div');
    col.className = 'col-md-4';
    col.innerHTML = `
      <div class="card gk-card ${borderCls} h-100">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-start">
            <strong style="font-size:.85rem">${titleCase(fid)}</strong>
            <span class="gk-badge ${badge.cls}">${badge.text}</span>
          </div>
          <div class="sub mt-2" style="font-size:.72rem;color:#64748b">
            Score: ${f.score.toFixed(2)} · Confidence: ${(f.base_conf * 100).toFixed(0)}%
            <span class="src-badge ${sourcing.cls} ms-1">${sourcing.text}</span>
          </div>
          <div class="gk-source">${f.source}</div>
        </div>
      </div>`;
    container.appendChild(col);
  });
...
// ═══════════════════════════════════════════════════════════
// MAIN COST CALCULATION (item 2 + 3)
// ═══════════════════════════════════════════════════════════
...
function renderMetricCards(data) {
  const [aPoint, aLow, aHigh] = data.trajectory_a_per_person_year;
