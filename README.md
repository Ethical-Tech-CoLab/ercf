...
- **Evacuation Decision Analysis** — break-even chart: compares one-time evacuation cost vs cumulative daily in-zone assistance cost; identifies the day after which evacuation becomes the lower-cost option. Includes a cost-of-delay note ("each day of delay costs an additional $X and shifts the break-even forward by approximately 1 day")
- **Cumulative Assistance Cost chart** — day-by-day projection of the full in-zone assistance cost (supply delivery + emergency extraction + field medical + vulnerable population support), not just the flat survival baseline
...
- **Break-even Analysis** — compares two options: (A) evacuate now + daily assistance for remaining population vs (B) no evacuation with full population assistance. `evacCost` is the full mobilization cost (`resources.totalCost` for ground, the alt-mode API's full-population quote for air/walking) — resources are mobilized for the whole population regardless of how many actually board, not a marginal per-evacuee recompute. Break-even = `evacCost / (dailyCostB - dailyCostA)`. Daily costs derived from the Assistance Cost module (access overhead, terrain, extraction, medical). Special case: 0% remaining shows evacuate-all vs no-evacuation comparison using `BASE_DAILY[level] × population` as the no-evacuation baseline.
