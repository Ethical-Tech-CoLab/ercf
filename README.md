...
- The D3 (Authorization) scale runs one direction in S4.3/S5.4 and the opposite in S7.2/S8.4/S9.3/S9.4, so a core input may be wired backwards. — **Confirmed and documented; not yet fixed.** The contradiction is in the code, not only the prose. `historical_data.py` stores *low* D3 for blocked-consent cases (Aleppo 2.0; Mariupol, Srebrenica, Gaza 1.0–1.5) and the mortality model matches it (`(5 − D3) × D4 / 5`, siege on `D3 ≤ 2.0`). But the composite risk score adds `D3 × 0.15` and Feasibility computes `6 − D3`, both assuming the opposite — so those two outputs understate risk exactly where consent has failed. **D7 has the same inversion**, which the review did not catch. No numbers have been changed pending a decision on which convention is authoritative; the composite risk score and Feasibility sub-index should not be relied on meanwhile. See the erratum at ERCF-Paper.md S4.3 and the warning comments in `calculators.py`.
